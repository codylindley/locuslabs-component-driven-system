describe('Background Image', () => {
	// Note the use of `before`
	before(() => {
		// Visit the storybook iframe page once per file
		cy.visitStorybook();
	});

	// Note the use of `beforeEach`
	beforeEach(() => {
		// The first parameter is the category. This is the `title` in CSF or the value in `storiesOf`
		// The second parameter is the name of the story. This is the name of the function in CSF or the value in the `add`
		// This does not refresh the page, but will unmount any previous story and use the Storybook Router API to render a fresh new story
		cy.loadStory(
			'MOL Components/Desktop/Primitive Components/Fullscreen Background Image',
			'WithContent'
		);
	});

	it('Displays content properly', () => {
		cy.get('[data-cy=FullscreenBackgroundImage]')
			.should('exist')
			.and('have.css', 'background-attachment', 'fixed')
			.and(
				'have.css',
				'background-image',
				'url("https://locuslabs.com/wp-content/uploads/2020/10/home-hero-bg-no-lines-1920x1080.png")'
			)
			.and('have.css', 'background-position', '50% 50%')
			.and('have.css', 'background-size', 'cover')
			.children('[data-cy=Box]')
			.should('exist')
			.and('have.text', 'Sample Content');
	});
});
