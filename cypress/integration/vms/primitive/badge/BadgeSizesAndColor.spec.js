describe('Badge Component', () => {
	...
	beforeEach(() => {
		...
		cy.loadStory(
			'VMS Components/Primitive Components/Badge',
			'BadgeSizesAndColor'
		);
	});
	...
	it('Story: Badge Sizes And Color - xl', () => {
		cy.get('[data-cy=Badge]:eq(4) [data-cy=Box]:eq(0) svg')
			.should('exist') // is in dom
			.and('have.css', 'height', '32px')
			.and('have.css', 'width', '32px');
		cy.get('[data-cy=Badge]:eq(4) [data-cy=Box]:eq(0) svg path')
			.should('exist') // is in dom
			.and('have.attr', 'stroke', 'white')
			.and('have.attr', 'fill', 'black');
		cy.get('[data-cy=Badge]:eq(4) [data-cy=Icon]')
			.should('exist') // is in dom
			.and('have.attr', 'color', 'white');
	});
});
