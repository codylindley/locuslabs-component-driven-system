describe('Select Component', () => {
	...
	beforeEach(() => {
		...
		cy.loadStory('VMS Components/Primitive Components/Select', 'Searchable');
	});
	...
	it('User can search option', () => {
		...
		cy.get('[data-cy=SelectControl]').click({ force: true });
		cy.get('[data-cy=Option]').its('length').should('be.gt', 1);

		// check if selection can be changed
		cy.get('[data-cy=SelectInput]').type('ore');
		cy.get('[data-cy=Option]').eq(0).click({ force: true });
		cy.get('[data-cy=SelectSingleValue]').should('contain', 'oregano');
