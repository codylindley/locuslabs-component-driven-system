...
The toggle switch allows users to choose between two mutually exclusive options (i.e. settings and user preferences). One option should always enabled by default.
	...
	items={[
		{
			...
			description: <p>component label.</p>,
		},
		{
			propName: 'enabled',
			type: '<boolean>',
			defaultValue: 'false',
			description: <p>component status.</p>,
		},
		{
			propName: 'onChange',
			type: '<function>',
			defaultValue: 'undefined',
			description: <p>The function to be called when control status change.</p>,
		},
	]}
/>
...
see <LinkTo kind="locuslabs-ui-components-common-props-common-styling-props--page#common-props">Common Props</LinkTo>
