...
<ComponentProps
	...
	isActive: <boolean>*,
			...
			description: (
				<>
					<p>
						Array of controls (objects). If the array has more than two
						elements, it will take only the first two.
					</p>
					<p>
						where:
						<br />
						<code>text</code>: control text
						<br />
						<code>isActive</code>: set to <code>true</code> if the control
						should be selected.
					</p>
				</>
			),
