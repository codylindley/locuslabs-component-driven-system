...
The tab menu allows use to navigate between groups of content that are related. Only one tab can be selected at a time, but the first one is active by default.
	...
	items={[
		{
			...
			type: (
				<DocNestedProp>
					{`[{ label: <string>*, hasUnsavedChanges: <string> }]`}
				</DocNestedProp>
			),
			defaultValue: 'undefined',
			description: (
				<p>Label's and unsaved changes indicator's displayed in each Tab.</p>
			),
		},
		{
			...
			type: '<number>',
			defaultValue: 'undefined',
			description: <p>Current index tab.</p>,
		},
		{
			propName: 'onChange',
			type: '<function>',
			...
			description: (
				<p>
					The function to be called when control tab change with current index
					tab as parameter.{' '}
				</p>
			),
		},
		{
			...
			description: (
				<p>
					The <code>aria-label</code> property is useful when there is no
					visible text content that will serve as an appropriate accessible
					name. Upon tab selection the <code>aria-selected</code> property will
					be set to <code>true</code> on the tab element. More info about{' '}
					<code>wai-aria</code> props{' '}
					<a href="https://www.w3.org/TR/wai-aria-practices/#naming_with_aria-label">
						here
					</a>
					.
				</p>
...
<ComponentStylingProps styleProps={['space', 'layout']} />
