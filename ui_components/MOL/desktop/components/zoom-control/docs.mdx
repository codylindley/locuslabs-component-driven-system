...
Zoom a map using a zoom in and zoom out button located on top of the map in the lower right-hand corner.
	...
	items={[
		{
			...
			description: (
				<p>The function to be called when the control is zoomed out.</p>
			),
		},
		{
			propName: 'onZoomIn',
			...
			defaultValue: 'undefined',
			description: (
				<p>The function to be called when the control is zoomed in.</p>
			),
		},
		{
		...
		{
			propName: 'zoomInTooltipContent',
			type: '<string>',
			defaultValue: 'Zoom In',
			description: (
				<p>
					The content used for the tooltip that appears on the zoom in button.
				</p>
			),
		},
		{
			propName: 'zoomOutTooltipContent',
			type: '<string>',
