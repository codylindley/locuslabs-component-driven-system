...
The desktop compass is used to indicate true North to the viewer while they are looking at the map.

The compass gives the consumer the ability to reset their current rotation so they can easily view the map oriented North.
...
<ComponentProps
	items={[
		...
		},
		{
			...
			type: '<string>',
			defaultValue: '"Reset Map Rotation"',
			description: (
				<p>
					The content used for the tooltip that appears on the compass button.
				</p>
			),
		},
	]}
