...
const PinStyled = styled('div')`
	...
	width: ${ifProp('small', '34px', '42px')};
	height: ${ifProp('small', '47px', '56px')};
	position: relative;
...
const Pin = forwardRef((props, ref) => {
	...
	return (
		<PinStyled ref={ref} {...props}>
			<Box position="absolute">
				<svg
					width={props.small && props.small !== false ? '34px' : '42px'}
					height={props.small && props.small !== false ? '47px' : '56px'}
					viewBox="0 0 42 56"
					fill="none"
				...
				>
					...
					<path
						...
						fill={
							props.border || themeContext.colors.miscBadgeIconStroke || 'black'
						}
					/>
				</svg>
			</Box>
			...
			<Box
				position="absolute"
				top={props.small && props.small !== false ? '1px' : '0px'}
				left="1px"
			>
				<Icon
					color={
					...
					iconSize={props.small && props.small !== false ? 'md' : 'xl'}
