...
The desktop logo component is a container that displays a customer’s svg logo in the Exploration UI. It sits above the search input facet in the Search component. The customer may also choose to not display a logo in which case the whole Logo component will be removed from the Search component.
	...
	items={[
		{
			...
			description: <p>Logo image url in svg format.</p>,
		},
		{
			propName: 'alt',
			type: '<string>*',
			defaultValue: "'Venue logo'",
			description: <p>Logo image alt</p>,
		},
		{
			propName: 'logoHeight',
			type: '<number>',
			...
			description: (
				<p>
					...
					will be used. Logo height will be adjusted so it is at least 28px high
					and at most 100px high.
				</p>
			),
		},
		{
			...
			defaultValue: 'center',
			description: <p>Use to change Logo alignment.</p>,
		},
	]}
/>
