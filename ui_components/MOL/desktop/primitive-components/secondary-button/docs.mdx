...
The desktop secondary button, similar to the primary button, is a styled link that but is less utilized and enables the user to perform secondary actions. Secondary buttons are the alternative we give users to the primary action.
...
<ComponentProps
	items={[
		...
		{
			propName: 'iconToTheRight',
			type: '<boolean>*',
			defaultValue: 'undefined',
			description: (
				<p>
					Moves the icon from the left of the text in the button to the right of
					the text in the button (excluding stacked secondary button).
				</p>
			),
		},
		{
			propName: 'type',
			type: "'normal' || 'stacked' || 'stackedWide' || 'small'**",
			defaultValue: 'normal',
			description: <p>The type of secondary button</p>,
		},
		{
			propName: 'disabled',
			type: '<boolean>',
