...
The footer is used to display the customer’s social media accounts. Each label contains an icon and account name. The footer will be preconfigured for customer.
	...
	label: <string>*,
			...
			defaultValue: (
				<DocNestedProp>
					{`[{
	icon: 'need.a.icon',
	label: 'footer items missing !!!',
}]`}
				</DocNestedProp>
			),
			description: <p>Icon's and label's displayed in footer.</p>,
		},
		{
			propName: 'type',
			type: "'editor' || 'display'",
			defaultValue: 'display',
			description: <p>Used to change size of footer.</p>,
		},
	]}
/>
