...
The walk time matrix widget is used to display the nearby locations that the customer chooses to display on their map. It contains section labels with their nearby location sublabels and estimated walk time to each location.
...
<ComponentProps
	...
	items: [{
			...
			defaultValue: (
				<DocNestedProp>
					{`[{
	label: 'Section label missing',
	items: [{
			locationLabel: 'Location label missing',
			walkTime: 'Walk time missing',
		}],
}]`}
				</DocNestedProp>
			),
