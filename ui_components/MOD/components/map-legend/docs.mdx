...
The Map Legend is used as as visual explanation of the categories and/or Points Of Interest (POIs) that are being shown on the map. Each legend item will contain a category badge with a label defined by the customer.
Customers have the ability to define up to 6 categories and/or POIs to highlight on the map.
	...
	label: <string>*,
			...
			defaultValue: (
				<DocNestedProp>
					{`[{
	badge: 'need.an.icon',
	label: 'legend items missing !',
}]`}
				</DocNestedProp>
			),
			description:
				'Array of map legend items. Maximum 6 items will be displayed.',
		},
		{
			propName: 'type',
			type: "'editor' || 'display'",
			defaultValue: 'display',
			description: 'Used to change size of map legend.',
		},
	]}
/>
