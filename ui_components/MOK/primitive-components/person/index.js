...
const PersonStyled = styled('svg')`
	...
	${compose(flexbox, grid, layout, position, shadow, space, display)};
`;
...
const StyledText = styled(Text)`
	color: ${theme('colors.programmaticPinMarkersText', '#000000')};
	text-shadow: ${css`
		${theme('colors.programmaticPinMarkersStroke', '#FFFFFF')} -1px -1px 1px,
		${theme('colors.programmaticPinMarkersStroke', '#FFFFFF')} 1px -1px 1px,
  		${theme('colors.programmaticPinMarkersStroke', '#ffffff')} -1px 1px 1px,
    	${theme('colors.programmaticPinMarkersStroke', '#FFFFFF')} 1px 1px 1px
	`};
...
const Person = forwardRef((props, ref) => {
	...
	const { primaryLabel, secondaryLabel, ...omittedProps } = props;
	const size = primaryLabel ? 'md' : 'sm';
	...
	return (
		<Box display="flex" alignItems="center">
			<PersonStyled
				ref={ref}
				{...omittedProps}
				width={personSize[size].width || 20}
				height={personSize[size].height || 50}
				viewBox="0 0 20 50"
			...
			>
				...
				<path
					...
					fill={
						props.fill ||
						themeContext.colors.miscWayfindingStartColor ||
						'black'
					}
				/>
				<path
				...
				<path
					fill={
						props.border ||
						themeContext.colors.miscWayfindingStartStroke ||
						'black'
					}
					fillRule="evenodd"
					clipRule="evenodd"
			...
			{primaryLabel && !secondaryLabel && (
				<StyledText
					...
					color={
						props.color ||
						themeContext.colors.programmaticPinMarkersText ||
						'black'
					}
					fontWeight="lg"
				>
