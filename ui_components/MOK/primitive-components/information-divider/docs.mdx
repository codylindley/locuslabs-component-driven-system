...
The information divider is used to visually separate information to help users understand how the content that is being displayed is organized.
...
<ComponentProps
	items={[
		{
			propName: 'type',
			type: "'nearbyResults' || 'otherResults' || 'poiDetail'",
			defaultValue: 'nearbyResults',
