...
The status indicator is a component that is displayed in the header of the POI Detail and in the Search POI Row to indicate to the user the current status to a specific POI (i.e. Open, Closed, Full, etc.)
...
<ComponentProps
	items={[
		{
			propName: 'type',
			type: `'default' || 'info' || 'warning' || 'alert' || 'active' ||
						'inactive'`,
			defaultValue: 'default',
			description: <p> The type status indicator.</p>,
		},
		{
			propName: 'pulsingStatusOn',
			type: '<boolean>',
			defaultValue: 'undefined',
			description: (
				<p>
					Show animating pulsing circle in front of text in status indicator.
				</p>
			),
		},
	]}
