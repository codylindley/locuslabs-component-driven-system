...
The primary navigation button is a styled link that is shown in the Navigation Bar at the bottom of the screen to grab the user’s attention in order to drive their attention to perform primary navigation actions within the product (i.e. Get Started & Search).
...
<ComponentProps
	items={[
		...
		},
		{
			...
			description: <p>Primary Navigation Button type.</p>,
		},
		{
			propName: 'primaryLabel',
			type: '<string>',
			defaultValue: 'undefined',
			description: <p>Primary label's text.</p>,
		},
		{
			propName: 'secondaryLabel',
			type: '<string>',
			defaultValue: 'undefined',
			description: <p>Secondary label's text</p>,
		},
		{
			propName: 'onClick',
			type: '<function>',
			...
			description: (
				<p>
					The function to be called when the primary navigation button is
					clicked.
				</p>
			),
		},
		{
			propName: 'onInfoButtonClick',
			type: '<function>',
			defaultValue: 'undefined',
			description: (
				<p>
					The function to be called when the info button (only in admin mode) is
					clicked.
				</p>
			),
		},
