...
The scrolling navigation button is a styled link that is shown in the Navigation Bar in the bottom of the screen that allows the user to navigate through the list of route segments that the user must take to get to their end POI destination.
...
<ComponentProps
	items={[
		...
		},
		{
			...
			description: (
				<p>
					The function to be called when some buttons are clicked. Direction (
					<code>{'left'}</code> or <code>{'right'}</code>) will be passed as a
					param.
				</p>
			),
		},
